...
public abstract class FieldParseInfo {
	 ...
	 * @param t The ISO8583 type that is required
	 * @param len The length (useful for ALPHA, BINARY and NUMERIC types)
	 * @param encoding The string encoding to use
	 * @param useDateTimeApi Whether to use the new Java date/time API or not.
	 */
	public static FieldParseInfo getInstance(IsoType t, int len, String encoding, boolean useDateTimeApi) {
		FieldParseInfo fpi = null;
		if (t.isDateTimeType()) {
			if (useDateTimeApi) {
				fpi = TemporalParseInfo.createParserForType(t);
			} else {
				fpi = DateTimeParseInfo.createParserForType(t);
			}
		} else if (t == IsoType.ALPHA) {
