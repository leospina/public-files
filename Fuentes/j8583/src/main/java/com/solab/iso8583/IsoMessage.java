...
public class IsoMessage {
     ...
     * real fields go from 2 to 128.
     */
    public <T> Optional<T> objectValue(int field) {
        @SuppressWarnings("unchecked")
        IsoValue<T> v = fields[field];
        return v == null ? Optional.empty() : Optional.ofNullable(v.getValue());
    }

    /** Returns the IsoValue for the specified field.
     * @param field The field number. First real field is 2.
     */
	@SuppressWarnings("unchecked")
    ...
    /** Returns the IsoValue for the specified field, wrapped in an Optional. First real field is 2. */
    @SuppressWarnings("unchecked")
    public <T> Optional<IsoValue<T>> field(int field) {
        return Optional.ofNullable(fields[field]);
    }
    ...
    public ByteBuffer writeToBuffer(int lengthBytes) {
    	...
    	byte[] data = writeData();
    	var buf = ByteBuffer.allocate(lengthBytes + data.length + (etx > -1 ? 1 : 0));
    	if (lengthBytes > 0) {
    		int l = data.length;
    		if (etx > -1) {
    			l++;
    ...
    public byte[] writeData() {
    	...
    	if (binaryHeader || binBitmap) {
            ...
            for (int i = 0; i < lim; i++) {
                ...
                if (bs.get(pos++))
                    nibble |= 1;
                bout.write(HexCodec.HEX[nibble]);
            }
            if (forceStringEncoding) {
                final String _hb = bout.toString();
                bout = bout2;
                try {
