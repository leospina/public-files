...
import com.solab.iso8583.util.DateFormats;
...
public enum IsoType {
	...
	LLLVAR(false, 0),
	/** A date in format YYYYMMddHHmmss (week year, not calendar year) */
	DATE14(false, 14),
    ...
    public static final Set<IsoType> VARIABLE_LENGTH_BIN_TYPES = Set.copyOf(EnumSet.of(LLBIN, LLLBIN, LLLLBIN, LLBCDBIN, LLLBCDBIN, LLLLBCDBIN));
	...
	public boolean isDateTimeType() {
		return this == DATE4 || this == DATE6 || this == DATE10 || this == TIME || this == DATE_EXP || this == DATE12 || this == DATE14;
	}
	...
	public String format(final Date value, final TimeZone tz) {
		if (tz == null) {
			return format(value.toInstant().atZone(ZoneId.systemDefault()));
		}
        return format(value.toInstant().atZone(tz.toZoneId()));
	}

	public String format(final Temporal value) {
		final DateTimeFormatter sdf = DateFormats.FORMATS.get(this);
		if (sdf == null) {
