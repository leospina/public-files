...
public class MessageFactory<T extends IsoMessage> {
	...
	private boolean useDateTimeApi;

	/** When set to true, specifies that the date/time types should be handled with the new Java Date/Time API,
	 * using ZonedDateTime, LocalDate, LocalTime.
	 */
	public void setUseDateTimeApi(boolean flag) {
		useDateTimeApi = flag;
	}
	public boolean isUseDateTimeApi() {
		return useDateTimeApi;
	}
	...
	public T newMessage(int type) {
        ...
        if (setDate) {
            if (m.hasField(7)) {
                ...
                m.updateValue(7, useDateTimeApi ? ZonedDateTime.now() : new Date());
            } else if (useDateTimeApi) {
				var now = new IsoValue<>(IsoType.DATE10, ZonedDateTime.now(TemporalParseInfo.getDefaultZoneId()));
				m.setField(7, now);
			} else {
                var now = new IsoValue<>(IsoType.DATE10, new Date());
	...
	/** Sets the ZoneId for the specified FieldParseInfo, if it's needed for parsing dates with the new date/time API.
	 * @param messageType The type of message for the parsing guide, e.g. 0x200
	 * @param field The field for which the ZoneId is to be set. Pass 0 to set the default ZoneId.
	 * @param zoneId The ZoneId to set in the field (or as default).
	 */
	public void setZoneIdForParseGuide(int messageType, int field, ZoneId zoneId) {
		if (field == 0) {
			TemporalParseInfo.setDefaultZoneId(zoneId);
			return;
		}
		Map<Integer, FieldParseInfo> guide = parseMap.get(messageType);
		if (guide != null) {
			FieldParseInfo fpi = guide.get(field);
			if (fpi instanceof TemporalParseInfo) {
				((TemporalParseInfo<?>) fpi).setZoneId(zoneId);
				return;
			}
		}
		log.warn("Field {} for message type {} is not for dates, cannot set ZoneId",
				field, messageType);
	}

    /** Sets the timezone for the specified FieldParseInfo, if it's needed for parsing dates.
	 * @param messageType The type of message for the parsing guide, e.g. 0x200
	 * @param field The field for which the TimeZone is to be set. Pass 0 to set the default TimeZone.
	 * @param tz The TimeZone to set in the field (or as default).
	 */
	...
	public T parseMessage(byte[] buf, int isoHeaderLength, boolean binaryIsoHeader)
		...
		if (index == null) {
			String errmsg = String.format("ISO8583 MessageFactory has no parsing guide for message type %04x [%s]",
					type, new String(buf));
			log.error(errmsg);
		...
		if (abandon) {
			throw new ParseException("ISO8583 MessageFactory cannot parse fields "+ j, 0);
		}
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						m.setField(i, val);
						if (val != null) {
							if (val.getType() == IsoType.NUMERIC || val.getType().isDateTimeType()
	...
	public void setParseMap(int type, Map<Integer, FieldParseInfo> map) {
		...
		var index = new ArrayList<>(map.keySet());
